...
var map = new ol.Map({
    ...
    layers: layersList,
    view: new ol.View({
        extent: [526466.494404, 6851774.694609, 561650.845843, 6881032.568728], maxZoom: 28, minZoom: 1
    })
});
...
map.getView().fit([526466.494404, 6851774.694609, 561650.845843, 6881032.568728], map.getSize());
        ...
        if (feature instanceof ol.Feature) {
            ...
            if (count == 1) {
                ...
                if (typeof clusteredFeatures !== "undefined") {
                    ...
                    currentFeatureKeys = currentFeature.getKeys();
                    if (doPopup) {
                        // var headTxt = currentLayer.getElementsByClassName.toString();
                        popupText = '<h3>Een reis in minuten</h3>'+ headTxt + '<table>';
                        for (var i=0; i<currentFeatureKeys.length; i++) {
                            if (currentFeatureKeys[i] != 'geometry') {
                                popupField = '';
                                if (layer.get('fieldLabels')[currentFeatureKeys[i]] == "inline label") {
                                ...
                                if (layer.get('fieldImages')[currentFeatureKeys[i]] != "Photo") {
                                    popupField += (currentFeature.get(currentFeatureKeys[i]) != null ? Math.round(Autolinker.link(String(currentFeature.get(currentFeatureKeys[i])))/60) + '</td>' : '');
                                } 
                                else {
                                    popupField += (currentFeature.get(currentFeatureKeys[i]) != null ? '<img src="images/' + currentFeature.get(currentFeatureKeys[i]).replace(/[\\\/:]/g, '_').trim()  + '" /></td>' : '');
                                }
